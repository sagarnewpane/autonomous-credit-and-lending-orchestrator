...
def optimize_image_for_ocr(img):
    ...
    # This turns the image into strict Black and White
...
def run_ocr(image_path,lang='nep+eng'):
    ...
    # Step 1: Detect orientation and rotate
    try:
        osd = pytesseract.image_to_osd(img)
        rotation_angle = int(re.search(r'(?<=Rotate: )\d+', osd).group(0))
        if rotation_angle != 0:
            # Rotate image to correct upright position
            (h, w) = img.shape[:2]
            center = (w // 2, h // 2)
            M = cv2.getRotationMatrix2D(center, -rotation_angle, 1.0)
            img = cv2.warpAffine(img, M, (w, h))
    except:
        pass # If OSD fails, proceed with original

    # Step 2: Clean the image
    processed_img = optimize_image_for_ocr(img)

    # Step 3: Run OCR once
    # Use --oem 3 (Default) and --psm 3 (Fully automatic page segmentation)
...
def parse_documents(image_paths: List[str], document_types: List[str] = None, debug: bool = False) -> Dict:
    ...
    tax_result = None
    citizenship_full_data = None
    lalpurja_full_data = None
    tax_full_data = None
    ...
    return {
        ...
        "raw_extracted_data": {
            "citizenship": citizenship_full_data,
            "lalpurja": lalpurja_full_data,
            "tax_clearance": tax_full_data
        }
    }
...
if __name__ == "__main__":
    ...
    # print(extract_citizenship_fields(run_ocr(image_paths[0])))
